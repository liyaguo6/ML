...
# input_array = np.arange(1,49,1).reshape(3,4,4)
# kernel_array = np.array([[1,0],[0,1]])
# bias=1
# ouput_array = np.zeros((4,4))
# print(input_array)
# print(kernel_array)
# # print(output_array)
#
# conv(input_array,kernel_array,output_array,1,bias)
...
# input_array = np.arange(-10,38,1).reshape(3,4,4)
class ConvLayer(object):
    ...
    def forward(self, input_array):
        ...
        for f in range(self.filter_number):
            ...
            # print(self.output_array[f])
            # print(self.stride)
            # print(filter.get_bias())
            conv(self.padded_input_array,
                 filter.get_weights(), self.output_array[f],
                 self.stride, filter.get_bias())
        element_wise_op(self.output_array,
                        self.activator.forward)  ##激活函数relu
    ...
    def bp_sensitivity_map(self, sensitivity_array,
        ...
        for f in range(self.filter_number):
            ...
            flipped_weights = np.array(list(map(
                ...
                filter.get_weights())))   #上一层的敏感误差等于本层的卷积核进行180度翻转后，与这一层的误差敏感项矩阵围补零后的矩阵做卷积运算后
        ...
        self.delta_array *= derivative_array    #公式14
        # print(self.delta_array)

    def bp_gradient(self, sensitivity_array):
        # 处理卷积步长，对原始sensitivity map进行扩展 计算代码梯度
        expanded_array = self.expand_sensitivity_map(
            sensitivity_array)
        for f in range(self.filter_number):
            # 计算每个权重的梯度
            filter = self.filters[f]
            print('padded_input_array')
            # print(self.padded_input_array)
            for d in range(filter.weights.shape[0]):
                conv(self.padded_input_array[d],
                     expanded_array[f],
                     filter.weights_grad[d], 1, 0)
            print(filter.weights_grad)   #权重的梯度 =本层的敏感误差对输入的padded_input_array做卷积
            # 计算偏置项的梯度
...
def init_test():
    ...
    cl = ConvLayer(5, 5, 3, 3, 3, 1, 1, 2, ReluActivator(), 0.001)
